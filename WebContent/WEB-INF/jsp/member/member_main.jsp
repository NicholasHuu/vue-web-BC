<%@ page language="java" import="java.util.*" pageEncoding="utf-8"%>
  ...
  <body class="${theme } theme-wrapper">
    <div class="wrapper">
      <div class="panel-header">
        <% if ("v2".equals(request.getSession().getAttribute("version"))) { %>
            <div class="promo-nav-bg"></div>
            <div class="inner clear">
                <div class="user-info clearfix">
                    <img class="left" src="${ctx}/member/getUserTypeImg?id=${webUser.userType}" alt="" class="avatar" />
                    <div class="ui-wrap left">
                        <ul class="c1">
                            <li><label>用户名</label>：${webUser.userName}</li></li>
                            <li><label>姓&nbsp;名</label>：${webUser.userRealName}</li>
                            <li><label>等&nbsp;级</label>：
                            <font color="red">${userType.typeLevel}
                	<c:if test="${!empty showName}">
                        ...
                        <ul class="c2">
                            <li><label>余&nbsp;额：</label><span>${webUser.userMoney}</span></li>
                        </ul>
                        <ul class="c3">
                            <li class="security-tips">
                                ...
                                <h6 class="">时&nbsp;间：<span class="font-hei "><fmt:formatDate value="${uc.lastLogonTime}" pattern="yyyy/MM/dd HH:mm:ss"/></span></h6>
                    ...
                    <div class="ui-links right">
                        <ul class="c4">
                            ...
                            <li class="qlinks">
                                <a class="promo-link" href="javascript:void(0);" onclick="loginOut();return false;">退出</a>
                            </li>
                        </ul>
                    </div>
                    <div class="ui-contact-links">
                        <ul class="c5">
                                <a class="" href="${ctx}/member/main">会员首页</a>
                                <a class="btn-contact online-service-link" target="_blank"  href="javascript:void(0)">联系我们</a>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="promo-nav">
                    <div class="promo-nav-wrap clearfix">
                       <div class="promo-notice-con left" onclick="winOpenMessage('${ctx}/message/goGongGaoList','message','top=0,left=0,width=800,height=450,resizable=yes')">
                           <span>系统消息</span>
                       ...
                       <a class="promo-slider-con right" href="${ctx}/goPageCenter?code=promos">
                         <span>最近优惠</span>
  ...
  ">
          ...
          <div class="panel-head-promo slide-paging left">
            <li class="item item-2 ">
              ...
              <div class="body slide-paging-body no " style="font-size: 14px">
                <ul>
                  <c:forEach var="t" items="${lbPromoList}" varStatus="s">
                    <li>${t.pmsTitle } ${t.pmsSubTitle }</li>
                  </c:forEach>
          ...
          <div class="member-balance left" style="display: none;">
            <ul class="clear">
              ...
              <li class="item item-2 slide-paging">
                <h6 class="gray">最新优惠 <a href="${ctx}/goPageCenter?code=promos" class="link-more block">更多优惠</a>
              </h6>
                <div class="body slide-paging-body" style="font-size: 14px">
                  <ul>
                    <c:forEach var="t" items="${lbPromoList}" varStatus="s">
        ...
        <div class="switch-page switch-load-mod inner font-hei clear">
          ...
          <div class="panel-sidebar left">
            <ul class="switch-name switch-load-name">
              ...
              <!-- 传统扫码 -->
              <c:forEach items="${kjPay }" var="pay">
                <li class="item icon-2" onclick="goback('${ctx}/member/goKjPay?paytype=${pay.payType }')">${pay.payName }</li>
              </c:forEach>
              
              <!-- 在线扫码 -->
              <c:forEach items="${thirdPay }" var="pay">
                <li class="item icon-4" onclick="goback('${ctx}/member/paykjOnline?paytype=${pay.payType }&payid=${pay.thirdPayId }&kjid=${pay.id }')">${pay.payName }</li>
              </c:forEach>
              ...
              <li class="item icon-8" onclick="goback('${ctx}/member/goBankList')">银行卡管理</li>
          ...
          <div class="panel-content left">
            ...
            <iframe id="memberFrame" name="memberFrame" onload="resizeIframe(this)" src="${ctx}/${backUrl}" scrolling="no" frameborder="0" width="100%"   allowtransparency="true">
            </iframe>
            <!-- /group -->
          </div>
          <!-- /content -->
