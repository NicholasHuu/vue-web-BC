...
<div class="page-body">
	<div class="progress-body-item progress-body-step1">
		<div class="verify-item verify-bank s12" style="display:none;">
			...
			<div class="verify-body">
				<table>
					...
					<c:forEach var="item" items="${userBankList}"  varStatus="item_index">					
						<tr>
							<td class="pr20 ar gray-dark">账户${item.pxh}</td>
							<td>${item.bankType}</td>
		...
		<div class="sheet-mod">
			<div class="sheet-content mt10">
				<form id="form1" name="form1" action="${ctx}/member/saveWithdraw">
				    <input type="hidden" value="${config.withdrawMinPay }" name="min_withdraw"/>
					<table class="mt20 s14">
						...
						<tr>
							<td class="pr20 ar gray-dark">收款账户</td>
							<td>
								<select id="userBankId" name="userBankId" class="write b " style="width: 200px;">
									<c:forEach var="item" items="${userBankList}"  varStatus="item_index">		
										<option value="${item.id}">${item.bankType} ${item.bankCard}</option>
									</c:forEach>
								</select>
								<a class="button button-raised button-bank button-small" href="${ctx }/member/goBankList">银行卡管理</a>
							</td>
						</tr>
						<tr>
							...
							<td>
								...
								<span class="pl10 s12 gray">最低提款额为：${config.withdrawMinPay }元</span>
							</td>
						</tr>
						<tr>
