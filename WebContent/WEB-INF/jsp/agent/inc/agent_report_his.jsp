...
<form action="${ctx}/agent/agentReport" method="post" id="page_form">
		...
		<table width="100%" class="ag-form">
					...
					<th colspan="19">
						<div class="ag-form-head cf">
							<div class="ele-filter l">
								<ul class="cf">
									<li>
										期数:<select name="betQs" id="betQs" class="ui-select">
												<c:forEach var="item" items="${betQs}">
													<option value="${item}" <c:if test="${qs==item}">selected="selected"</c:if>>${item}</option>
												</c:forEach>
											</select>
									</li>
									<c:if test="${isAdmin }">
									<li> 代理商:<input name="agentName" type="text" class="ui-write" value="${agentName }" />
									</li>
									</c:if>
			...
			<tbody>
				...
				<tr>
					...
					<td><input class="ui-btn" onclick="goto_detail('${item.bet_user_agent}','${item.bet_qs}','zj')" type="button" value="详情" /></td>
					<td>
