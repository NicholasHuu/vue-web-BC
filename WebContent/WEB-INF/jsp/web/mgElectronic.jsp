						...
						<div class="ui-mod-body">
							<ul class="ui-group cf" id="_memu">
                                ...
                                <li class="subcat-drawer ui-item" style="height: 39px;">
                                    ...
                                    <!-- <div class="more-subcate">
                                        <c:forEach var="item" items="${electronicClass}">
                                           <a class="${gameType1 eq item.eleType ? 'ui-link-active':'' }" href="${ctx}/electronic?code=${item.eleFlat }&gameType1=${item.eleType}">${item.eleTypeName }</a>
                                        </c:forEach>
                                    </div> -->
                                </li>
                                <c:forEach var="item" items="${electronicClass}">
								...
								<div class="row-title">
								   <c:choose>
					                 <c:when test="${item.isFavourite eq 1 }">
					                     <img title="我的收藏"  src="${resourceRoot }/web/ybb/assets/img/slots/like-on.png" class="like-slot active" alt="" onclick="likeThisSlot(this, '${code}', '${item.remark}')"/>
					                 </c:when>
					                 <c:otherwise>
					                    <img title="我的收藏"  src="${resourceRoot }/web/ybb/assets/img/slots/like-off.png" class="like-slot" alt="" onclick="likeThisSlot(this, '${code}', '${item.remark}')"/>  
					                 </c:otherwise>
					               </c:choose>
  ...
  <div class="container">
    <c:if test="${gameType1 eq 'all'  || gameType1 eq null }">
      ...
      <div class="ui-group">
        ...
        <div class="ui-mod-body">
          <ul class="row cf">
            <c:forEach items="${mg_all['Table'] }" var="item">
              <li class="col-sm-2 ui-item">
                <c:choose>
                  <c:when test="${!empty webUser}">
                    <a href="javascript:void(0);" title="开始游戏" onclick="winOpen('${ctx}/forwardGame?gameType=mgElectronic&gameName=${item.eleGameId}&gameCode=${item.remark }',window.screen.width,window.screen.height,0,0,'game','mg');return false;" target="_blank" class="ui-link">${item.eleGameCnname }</a>
                  </c:when>
                  <c:otherwise>
      ...
      <div class="ui-group">
        ...
        <div class="ui-mod-body">
          <ul class="row cf">
            <c:forEach items="${mg_all['Slots'] }" var="item">
              <li class="col-sm-2 ui-item">
                <c:choose>
                  <c:when test="${!empty webUser}">
                    <a href="javascript:void(0);" title="开始游戏" onclick="winOpen('${ctx}/forwardGame?gameType=mgElectronic&gameName=${item.eleGameId}&gameCode=${item.remark }',window.screen.width,window.screen.height,0,0,'game','mg');return false;" target="_blank" class="ui-link">${item.eleGameCnname }</a>
      ...
      <!-- 分类 -->
      <div class="ui-group">
        <div class="ui-mod-head">新老虎机</div>
        <div class="ui-mod-body">
          <ul class="row cf">
            <c:forEach items="${mg_all['NewSlots'] }" var="item">
              <li class="col-sm-2 ui-item">
                <c:choose>
                  <c:when test="${!empty webUser}">
                    <a href="javascript:void(0);" title="开始游戏" onclick="winOpen('${ctx}/forwardGame?gameType=mgElectronic&gameName=${item.eleGameId}&gameCode=${item.remark }',window.screen.width,window.screen.height,0,0,'game','mg');return false;" target="_blank" class="ui-link">${item.eleGameCnname }</a>
      ...
      <div class="ui-group">
        ...
        <div class="ui-mod-body">
          <ul class="row cf">
            <c:forEach items="${mg_all['Video'] }" var="item">
              <li class="col-sm-2 ui-item">
                <c:choose>
                  <c:when test="${!empty webUser}">
                    <a href="javascript:void(0);" title="开始游戏" onclick="winOpen('${ctx}/forwardGame?gameType=mgElectronic&gameName=${item.eleGameId}&gameCode=${item.remark }',window.screen.width,window.screen.height,0,0,'game','mg');return false;" target="_blank" class="ui-link">${item.eleGameCnname }</a>
      ...
      <!-- 分类 -->
      <div class="ui-group">
        <div class="ui-mod-head">其他游戏</div>
        <div class="ui-mod-body">
          <ul class="row cf">
            <c:forEach items="${mg_all['Qt'] }" var="item">
              <li class="col-sm-2 ui-item">
                <c:choose>
                  <c:when test="${!empty webUser}">
                    <a href="javascript:void(0);" title="开始游戏" onclick="winOpen('${ctx}/forwardGame?gameType=mgElectronic&gameName=${item.eleGameId}&gameCode=${item.remark }',window.screen.width,window.screen.height,0,0,'game','mg');return false;" target="_blank" class="ui-link">${item.eleGameCnname }</a>
      ...
      <!-- 分类 -->
      <div class="ui-group">
        <div class="ui-mod-head">累积彩池</div>
        <div class="ui-mod-body">
          <ul class="row cf">
            <c:forEach items="${mg_all['Pot'] }" var="item">
              <li class="col-sm-2 ui-item">
                <c:choose>
                  <c:when test="${!empty webUser}">
                    <a href="javascript:void(0);" title="开始游戏" onclick="winOpen('${ctx}/forwardGame?gameType=mgElectronic&gameName=${item.eleGameId}&gameCode=${item.remark }',window.screen.width,window.screen.height,0,0,'game','mg');return false;" target="_blank" class="ui-link">${item.eleGameCnname }</a>
    ...
    <c:if test="${gameType1 eq 'NewSlots'}">
      <div class="ui-group">
        <div class="ui-mod-head">新老虎机</div>
        <div class="ui-mod-body">
          <ul class="row cf">
