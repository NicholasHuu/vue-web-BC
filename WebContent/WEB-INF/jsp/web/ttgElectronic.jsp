							...
							<c:forEach var="item" items="${result}" varStatus="status">
									...
									<c:choose>
										...
										<c:otherwise>
											<a href="javascript:void(0);" data-toggle="modal"
												...
												data-original="${item.eleGamePic}"
												style=" background-image: url(${resourceRoot}/web/ybb/assets/img/blank.png);background-size: 100%;background-position: 0px -6px;"></a>
										</c:otherwise>
									</c:choose>
									<!-- / 开始游戏判断 -->
								</div>
								<div class="row-title">
	                                ...
	                                <c:choose>
	                                 <c:when test="${item.isFavourite eq 1 }">
	                                     <img title="我的收藏" src="${resourceRoot }/web/ybb/assets/img/slots/like-on.png" class="like-slot active" alt="" onclick="likeThisSlot(this, '${code}', '${item.eleGameId}')"/>
	                                 </c:when>
	                                 <c:otherwise>
	                                    <img title="我的收藏" src="${resourceRoot }/web/ybb/assets/img/slots/like-off.png" class="like-slot" alt="" onclick="likeThisSlot(this, '${code}', '${item.eleGameId}')"/>   
	                                 </c:otherwise>
	                                </c:choose>
								...
								<div class="ac">
									<c:if test="${gameType1 eq 'Pot' }">
										<script src="http://www.tickerassist.co.uk/ProgressiveTickers/include/js/ProgressiveTickersControl.js?progid=${item.eleGameName}&font-color=yellow&font-family=verdana&showlogo=no&currency=CNY"></script>
										<script type="text/javascript">
											function reflashPot() {
  ...
  <div class="container">
    <c:if test="${gameType1 eq 'all'  || gameType1 eq null }">
      ...
      <div class="ui-group">
        ...
        <div class="ui-mod-body">
          <ul class="row cf">
            <c:forEach items="${ttg_all['Table games'] }" var="item">
              <li class="col-sm-2 ui-item">
                <c:choose>
                  <c:when test="${!empty webUser}">
                    <a href="javascript:void(0);" title="开始游戏" onclick="winOpen('${ctx}/forwardGame?gameType=ttg&gameName=${item.eleGameEnname}&gameId=${ item.eleGameId}&eleGameType=${item.eleGameType2 }',window.screen.width,window.screen.height,0,0,'game','ttg');return false;" href="javascript:void(0);" target="_blank" class="ui-link">${item.eleGameCnname }</a>
                  </c:when>
                  <c:otherwise>
      ...
      <div class="ui-group">
        ...
        <div class="ui-mod-body">
          <ul class="row cf">
            <c:forEach items="${ttg_all['Slot'] }" var="item">
              <li class="col-sm-2 ui-item">
                <c:choose>
                  <c:when test="${!empty webUser}">
                    <a href="javascript:void(0);" title="开始游戏" onclick="winOpen('${ctx}/forwardGame?gameType=ttg&gameName=${item.eleGameEnname}&gameId=${ item.eleGameId}&eleGameType=${item.eleGameType2 }',window.screen.width,window.screen.height,0,0,'game','ttg');return false;" href="javascript:void(0);" target="_blank" class="ui-link">${item.eleGameCnname }</a>
      ...
      <!-- 分类 -->
      <div class="ui-group">
        <div class="ui-mod-head">卡牌游戏</div>
        <div class="ui-mod-body">
          <ul class="row cf">
            <c:forEach items="${ttg_all['Card games'] }" var="item">
              <li class="col-sm-2 ui-item">
                <c:choose>
                  <c:when test="${!empty webUser}">
                    <a href="javascript:void(0);" title="开始游戏" onclick="winOpen('${ctx}/forwardGame?gameType=ttg&gameName=${item.eleGameEnname}&gameId=${ item.eleGameId}&eleGameType=${item.eleGameType2 }',window.screen.width,window.screen.height,0,0,'game','ttg');return false;" target="_blank" class="ui-link">${item.eleGameCnname }</a>
                  </c:when>
                  <c:otherwise>
                    <a href="javascript:void(0);" data-toggle="modal" data-target=".modal-login" title="登录" class="ui-link">${item.eleGameCnname }</a>
                  </c:otherwise>
      ...
      <div class="ui-group">
        ...
        <div class="ui-mod-body">
          <ul class="row cf">
            <c:forEach items="${ttg_all['Video Poker'] }" var="item">
              <li class="col-sm-2 ui-item">
                <c:choose>
                  <c:when test="${!empty webUser}">
                    <a href="javascript:void(0);" title="开始游戏" onclick="winOpen('${ctx}/forwardGame?gameType=ttg&gameName=${item.eleGameEnname}&gameId=${ item.eleGameId}&eleGameType=${item.eleGameType2 }',window.screen.width,window.screen.height,0,0,'game','ttg');return false;" href="javascript:void(0);" target="_blank" class="ui-link">${item.eleGameCnname }</a>
      ...
      <!-- 分类 -->
      <div class="ui-group">
        <div class="ui-mod-head">纸牌游戏</div>
        <div class="ui-mod-body">
          <ul class="row cf">
            <c:forEach items="${ttg_all['Soft games'] }" var="item">
              <li class="col-sm-2 ui-item">
                <c:choose>
                  <c:when test="${!empty webUser}">
                    <a href="javascript:void(0);" title="开始游戏" onclick="winOpen('${ctx}/forwardGame?gameType=ttg&gameName=${item.eleGameEnname}&gameId=${ item.eleGameId}&eleGameType=${item.eleGameType2 }',window.screen.width,window.screen.height,0,0,'game','ttg');return false;" href="javascript:void(0);" target="_blank" class="ui-link">${item.eleGameCnname }</a>
    ...
    <c:if test="${gameType1 eq 'Table games'}">
      <div class="ui-group">
        <div class="ui-mod-head">桌面游戏</div>
        <div class="ui-mod-body">
          <ul class="row cf">
    ...
    <c:if test="${gameType1 eq 'Card games'}">
      <div class="ui-group">
        <div class="ui-mod-head">卡牌游戏</div>
        <div class="ui-mod-body">
          <ul class="row cf">
    ...
    <c:if test="${gameType1 eq 'Soft games'}">
      <div class="ui-group">
        <div class="ui-mod-head">纸牌游戏</div>
        <div class="ui-mod-body">
          <ul class="row cf">
