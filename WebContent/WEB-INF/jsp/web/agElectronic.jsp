...
<body class="ybb-web ybb-slot-bd slot slot-ag slot-mg">
	...
	<div class="page-body">
		<div class="container cf">
	            <div class="ag-bnner clear">
	                <ul class="clear">
	                    <li class="left"><a 
	                    href="javascript:void(0)" 
	                    onclick="winOpen('${ctx}/forwardGame?gameType=ag&agGameType=6',window.screen.width,window.screen.height,0,0,'game','ag')">
	                    <img src="${resourceRoot }/web/ybb/assets/img/slots//fishing-banner.png" alt="" /></a></li>
	                    <li class="right">
	                    <c:choose>
	                        <c:when test="${empty webUser }">
	                            <a 
	                            href="javascript:void(0)" 
	                            onclick="javascript:alert('请先登录');">
	                            <img src="${resourceRoot }/web/ybb/assets/img/slots//ag-banner.png" alt="" /></a>
	                        </c:when>
	                        <c:otherwise>
	                            <a 
	                            href="javascript:void(0)" 
	                            onclick="winOpen('${ctx}/forwardGame?gameType=ag&agGameType=500',window.screen.width,window.screen.height,0,0,'game','ag');return false;">
	                            <img src="${resourceRoot }/web/ybb/assets/img/slots//ag-banner.png" alt="" /></a>
	                        </c:otherwise>
	                    </c:choose></li>
	                </ul>
								...
								<div class="row row-btn">
									<div class="col-sm-12 ui-item-son">
										...
										<c:choose>
											<c:when test="${!empty webUser}">
												<a href="javascript:void(0);" target="_blank"
													onclick="winOpen('${ctx}/forwardGame?gameType=ag&agGameType=${item.eleGameId}',window.screen.width,window.screen.height,0,0,'game','ag');return false;" href="javascript:void(0);"
...
<div class="ui-mod mod-slot-map">
  <div class="container">
    <c:if test="${gameType1 eq 'all'  || gameType1 eq null }">
      <!-- 分类 -->
      <div class="ui-group">
        <div class="ui-mod-head">老虎机游戏</div>
        <div class="ui-mod-body">
          <ul class="row cf">
            <c:forEach items="${ag_all['Slot'] }" var="item">
              <li class="col-sm-2 ui-item">
                <c:choose>
                  <c:when test="${!empty webUser}">
                    <a href="javascript:void(0);" title="开始游戏" onclick="winOpen('${ctx}/forwardGame?gameType=ag&gameName=${item.eleGameEnname}&gameId=${ item.eleGameId}&eleGameType=${item.eleGameType2 }',window.screen.width,window.screen.height,0,0,'game','ttg');return false;" href="javascript:void(0);" target="_blank" class="ui-link">${item.eleGameCnname }</a>
                  </c:when>
                  <c:otherwise>
      ...
      <div class="ui-group">
        ...
        <div class="ui-mod-body">
          <ul class="row cf">
            <c:forEach items="${ag_all['Table'] }" var="item">
              <li class="col-sm-2 ui-item">
                <c:choose>
                  <c:when test="${!empty webUser}">
                    <a href="javascript:void(0);" title="开始游戏" onclick="winOpen('${ctx}/forwardGame?gameType=ag&gameName=${item.eleGameEnname}&gameId=${ item.eleGameId}&eleGameType=${item.eleGameType2 }',window.screen.width,window.screen.height,0,0,'game','ttg');return false;" href="javascript:void(0);" target="_blank" class="ui-link">${item.eleGameCnname }</a>
      ...
      <div class="ui-group">
        ...
        <div class="ui-mod-body">
          <ul class="row cf">
            <c:forEach items="${ag_all['Video'] }" var="item">
              <li class="col-sm-2 ui-item">
                <c:choose>
                  <c:when test="${!empty webUser}">
                    <a href="javascript:void(0);" title="开始游戏" onclick="winOpen('${ctx}/forwardGame?gameType=ag&gameName=${item.eleGameEnname}&gameId=${ item.eleGameId}&eleGameType=${item.eleGameType2 }',window.screen.width,window.screen.height,0,0,'game','ttg');return false;" href="javascript:void(0);" target="_blank" class="ui-link">${item.eleGameCnname }</a>
